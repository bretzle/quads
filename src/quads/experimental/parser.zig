...
pub fn Pattern(comptime TokenType: type) type {
    return struct {
        ...
        pub fn create(typ: TokenType, match: Matcher) Self {
            return .{ .typ = typ, .match = match };
        }
    };
}
...
pub fn Tokenizer(comptime TokenTypeT: type, comptime patterns: []const Pattern(TokenTypeT)) type {
    return struct {
        ...
        pub fn create(source: []const u8, file_name: ?[]const u8) Self {
            return .{
                .source = source,
                .offset = 0,
                .current_location = .{
...
/// `TokenizerT` type of `Tokenizer(_, _)`
/// ignore_list is an array of token types
pub fn Parser(comptime TokenizerT: type, comptime ignore_list: anytype) type {
    const ignore_array: [ignore_list.len]TokenizerT.TokenType = ignore_list;

    return struct {
        const Self = @This();

        const TokenType: type = TokenizerT.TokenType;
        const Token: type = TokenizerT.Token;

        const Rule = fn (TokenType) bool;
        const State: type = TokenizerT.State;

        tokenizer: *TokenizerT,

        pub fn create(tokenizer: *TokenizerT) Self {
        ...
        pub fn nextToken(self: *Self) !?Token {
            while (try self.tokenizer.next()) |tok| {
                if (std.mem.indexOfScalar(TokenType, &ignore_array, tok.typ) == null) {
                    return tok;
                }
            }
            return null;
        ...
        pub fn accept(self: *Self, comptime rule: Rule) !Token {
            const state = self.saveState();
...
pub const Location = struct {
    ...
    pub fn advance(self: *Self, str: []const u8) void {
        for (str) |c| {
            if (c == '\n') {
                self.line += 1;
                self.column = 1;
    ...
    pub fn format(self: Self, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        if (self.source) |source_file| {
            try writer.print("{s}:{d}:{d}", .{ source_file, self.line, self.column });
        } else {
...
pub fn GenericToken(comptime T: type) type {
    meta.compileAssert(@typeInfo(T) == .@"enum", "T must be an enum", .{});
...
test "simple tokenization" {
    var tokenizer = TestTokenizer.create("10hellowhile10while", null);
    ...
    try testing.expectEqual(TestTokenType.number, number0.typ);
    try testing.expectEqual(TestTokenType.identifier, identifier.typ);
    try testing.expectEqual(TestTokenType.number, number1.typ);
    try testing.expectEqual(TestTokenType.keyword, keyword.typ);

    try testing.expectEqualStrings("10", number0.text);
    try testing.expectEqualStrings("hellowhile", identifier.text);
    try testing.expectEqualStrings("10", number1.text);
    try testing.expectEqualStrings("while", keyword.text);

    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 1 }, number0.location);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 3 }, identifier.location);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 13 }, number1.location);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 15 }, keyword.location);
}

test "invalid character" {
    var tokenizer = TestTokenizer.create("hello!", null);
    const identifier = (try tokenizer.next()) orelse return error.MissingToken;
    try testing.expectEqual(TestTokenType.identifier, identifier.typ);
    try testing.expectEqualStrings("hello", identifier.text);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 1 }, identifier.location);

    try testing.expectError(error.UnexpectedCharacter, tokenizer.next());
}

test "save/restore tokenization" {
    var tokenizer = TestTokenizer.create("hello\nworld", null);
    ...
    try testing.expectEqual(TestTokenType.identifier, id0.typ);
    try testing.expectEqual(TestTokenType.whitespace, ws0.typ);
    try testing.expectEqual(TestTokenType.whitespace, ws1.typ);
    try testing.expectEqual(TestTokenType.identifier, id1.typ);

    try testing.expectEqualStrings("hello", id0.text);
    try testing.expectEqualStrings("\n", ws0.text);
    try testing.expectEqualStrings("\n", ws1.text);
    try testing.expectEqualStrings("world", id1.text);

    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 1 }, id0.location);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 6 }, ws0.location);
    try testing.expectEqual(Location{ .source = null, .line = 1, .column = 6 }, ws1.location);
    try testing.expectEqual(Location{ .source = null, .line = 2, .column = 1 }, id1.location);
}

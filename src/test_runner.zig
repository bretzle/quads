...
pub fn main() !void {
    ...
    var printer = Printer{ .out = std.io.getStdErr().writer() };
...
const Printer = struct {
    ...
    pub fn print(self: Printer, comptime fmt: []const u8, args: anytype) void {
        std.fmt.format(self.out, fmt, args) catch @panic("OOM");
    }
    ...
    pub fn status(self: Printer, s: Status, comptime format: []const u8, args: anytype) void {
        ...
        self.out.writeAll(color) catch @panic("OOM");
        std.fmt.format(self.out, format, args) catch @panic("OOM");
        self.out.writeAll("\x1b[0m") catch @panic("OOM");
    }
};
...
const SlowTracker = struct {
    ...
    fn init(allocator: std.mem.Allocator, count: u32) SlowTracker {
...
pub fn panic(msg: []const u8, error_return_trace: ?*std.builtin.StackTrace, ret_addr: ?usize) noreturn {
    ...
    std.debug.FormattedPanic.call(msg, error_return_trace, ret_addr);
}
